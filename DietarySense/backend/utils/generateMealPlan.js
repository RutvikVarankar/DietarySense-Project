...
const generateMealPlan = async (user, duration = 7, preferences = {}) => {
  try {
    ...
    if (
      ...
      queryFilters.dietaryTags = userProfile.dietaryPreference;
    }
    ...
    for (let i = 0; i < duration; i++) {
      ...
      // Simple recipe selection - just pick random recipes
      const selectedRecipes = getRandomRecipes(matchingRecipes, 4); // breakfast, lunch, dinner, snack
      ...
      days.push({
        ...
        dayNumber: i + 1,
        meals: {
          breakfast: [{ recipe: selectedRecipes[0]._id }],
          lunch: [{ recipe: selectedRecipes[1]._id }],
          dinner: [{ recipe: selectedRecipes[2]._id }],
          snacks: [{ recipe: selectedRecipes[3]._id }],
        },
