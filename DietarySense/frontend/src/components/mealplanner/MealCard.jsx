...
const MealCard = ({ meal, mealType, dayIndex, mealIndex }) => {
  ...
  // Mock recipe data - replace with actual recipe data from backend
  ...
  const handleMarkConsumed = async () => {
    try {
      // API call to mark meal as consumed
      const response = await fetch(
        `http://localhost:5000/api/mealplans/${meal.planId}/days/${dayIndex}/meals/${mealType}/${mealIndex}/consume`,
        {
          method: "PUT",
          headers: {
